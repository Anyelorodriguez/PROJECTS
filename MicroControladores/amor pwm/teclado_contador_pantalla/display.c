...
#include "board.h" // se nececitan funciones que se llaman mas abajo (delay)
#include "display.h"
...
void visualizacion(int contador, int delay) // variables del main.c
{
		int umil 		= 0; 
		int	centena = 0;
		int decena 	= 0;  
		...
		{
			GPIO_PORTE_DATA_R = 0x10; // PE5 ON  PE4 OFF (prender unindades, apagar decenas)
			GPIO_PORTD_DATA_R = 0x00; // PD7 OFF PD6 OFF (apaga centenas, apaga miles)
			GPIO_PORTB_DATA_R = display(unidad); // sube a la funcion con el valor de "unidad" 
			                                     //retorna el valor de "salida" al gpio portB
			delay_ms(1);
