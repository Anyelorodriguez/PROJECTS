...
;			STR		R1, [R0] 	
			...
			LDR		R0, =GPIO_PORTD_LOCK_R ; PF Y PD TIENEN BLOCK 
			LDR		R1, =GPIO_LOCK_KEY
			STR		R1, [R0] 
...
;SUMA 1
DIGITOS		MOV		R4, #1		;CANTIDAD DE MILISEGUNDOS QUE VA A DURAR UN VALOR EN EL DISPLAY 
LOOP_DIS	
			...
			SUBS	R4, #0x01	
			BNE		LOOP_DIS
			CMP		R8, #0x01	;CONDICION CONTADOR ASCENDENTE
			BEQ		UNI
			CMP		R8, #0x02	;CONDICION CONTADOR DESCENDENTE
			BEQ		UNIR
			CMP		R8, #0x04	;CONDICION CONTADOR ASCENDENTE PARES DE A 2
			BEQ.W	UNI2
			CMP		R8, #0x08	;CONDICION CONTADOR DESCENDENTE IMPARES DE A 2
			BEQ.W	UNIR2
			B       INICIO
;CONTADOR ASCENDENTE
UNI 
...
;			BEQ		SUMA
			ADD		R1, #1
			ADD		R3, #1 ;contador para numero final
			STR 	R1, [R0]
			MOVW	R5, #0x1DAC ;se carga numero en registro R5
			CMP		R3, R5 ; se comparan para llegar a numero
			BEQ		BORRAR
			CMP 	R1, #10
			BEQ 	DEC
			B		INICIO
...
BORRAR	;funcion que devueleve todo a 0
			MOV		R3, #0
            MOV		R1,	#0
		    LDR 	R0, =CONTD
		    STR 	R1, [R0]
...
;CONDICION CONTADOR DESCENDENTE
UNIR 
            MOV		R1,	#9
			STR 	R1, [R0]
			LDR 	R0, =CONTA 
			...
			SUBS	R1, #1
			SUBS	R3, #1 ;contador para numero final
			STR 	R1, [R0]
			MOV		R5, #0xFFFFFFFF ;se carga numero en registro R5
			CMP		R3, R5 ; se comparan para llegar a numero
			BEQ		BORRARR
			CMP 	R1, #-1
			BEQ 	DECR
...
BORRARR	;funcion que devueleve todo a 0
			MOV		R3, #7595
            MOV		R1,	#7
		    LDR 	R0, =CONTD
		    STR 	R1, [R0]
...
; CONTADOR ASCENDENTE PARES DE A 2
UNI2 
            MOV		R1,	#0
			STR 	R1, [R0]
			LDR 	R0, =CONTA 
			LDR		R1, [R0]
			AND		R6,R1, #0x01
			CMP		R6,#0x01 ; se comparan para llegar a numero
			SUBEQ	R1, #1
			CMP		R6,#0x01
			SUBEQ	R3, #1
			ADD		R1, #2
			ADD		R3, #2 ;contador para numero final
			STR 	R1, [R0]
			MOVW	R5, #0x1DAC ;se carga numero en registro R5
			CMP		R3, R5 ; se comparan para llegar a numero
			BEQ		BORRAR
...
;CONDICION CONTADOR DESCENDENTE IMPARES DE A 2
UNIR2 
            MOV		R1,	#9
			STR 	R1, [R0]
			LDR 	R0, =CONTA 
			...
			AND		R6,R1, #0x01
			CMP		R6,#0x00 ; se comparan para llegar a numero
			ADDEQ	R1, #1
			CMP		R6,#0x00
			ADDEQ	R3, #1
			
			SUBS	R1, #2
			SUBS	R3, #2 ;contador para numero final
			STR 	R1, [R0]
			MOV		R5, #0xFFFFFFFF ;se carga numero en registro R5
			CMP		R3, R5 ; se comparan para llegar a numero
			BEQ		BORRARR
...
DISPLAY		CMP 	R0, #0x00 ; La siguiente funcion lee el valor del registro R0 y reliza el cambio de binario a 7 segmentos
			BEQ 	SEG0
			CMP 	R0, #0x01
			BEQ 	SEG1
			CMP 	R0, #0x02
